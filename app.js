...
// ── MIGRATION: ensure all keys exist on older saved states ──
...
if (!(‘apiKey’   in state)) state.apiKey   = ‘’;
...
// Show onboarding only if truly brand new (no profile AND no existing data)
const hasExistingData = state.habits.length || state.tasks.length || state.notes.length || state.sessions.length;
if (!state.profile && !hasExistingData) {
showOnboarding();
} else {
// Returning user without a profile — create a minimal one so greetings work
if (!state.profile) {
state.profile = { name: ‘’, birthday: ‘’, avatar: ‘🧠’, goals: [], joinDate: todayStr() };
save();
}
launchApp();
}
...
const d = h.completedDates?.includes(today);
return `<div class="habit-item ${d ? 'done' : ''}" onclick="toggleHabit(${i})"> <div class="habit-emoji">${h.emoji}</div> <div class="habit-info"> <div class="habit-name">${esc(h.name)}</div> <div class="habit-meta">${h.freq} &nbsp;·&nbsp; 🔥 ${h.streak || 0} day streak</div> </div> <div class="habit-check">✓</div> </div>`;
}).join(’’);
}
...
const system = `You are FlowMind's AI assistant — warm, sharp, encouraging. The user's name is ${name}. User data: Habits: ${habits}. Pending tasks: ${tasks}. Focus today: ${focus} minutes. Goals: ${goals}. Be concise (max 3 sentences unless asked more). Use 1-2 emojis. Address them by name naturally sometimes.`;
